...
export function createAssignment({ title, description, reference_text, class_name }) {
  const db = getDb()
  const result = db.prepare(`
    INSERT INTO assignments (title, description, reference_text, class_name)
    VALUES (?, ?, ?, ?)
  `).run(title, description || null, reference_text || null, class_name || null)
  return result.lastInsertRowid
}

export function updateAssignment(id, fields) {
  const db = getDb()
  const { title, description, reference_text, class_name, active } = fields
  db.prepare(`
    ...
    WHERE id = ?
  `).run(title ?? null, description ?? null, reference_text ?? null, class_name ?? null, active ?? null, id)
}
...
export function createSubmission(data) {
  ...
  const {
    ...
    feedback_en, feedback_cn, corrections
  } = data
...
export function getSubmissions({ class_name, date_from, date_to, min_score, max_score, limit = 200, offset = 0 } = {}) {
  const db = getDb()
  const conditions = []
  const params = []
...
export function getSubmission(id) {
  return getDb().prepare(`
    SELECT s.*, a.title as assignment_title, a.reference_text
    FROM submissions s
    LEFT JOIN assignments a ON s.assignment_id = a.id
    WHERE s.id = ?
  `).get(id)
